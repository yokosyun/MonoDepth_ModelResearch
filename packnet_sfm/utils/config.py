...
def set_checkpoint(config):
    ...
    if config.checkpoint.filepath is not "":
        ...
        config.checkpoint.monitor = os.path.join(
            "{}-{}".format(
    ...
    if model.logger and not model.config.wandb.dry_run:
        ...
        if model.config.checkpoint.filepath is not "":
            ...
            dirpath = os.path.join(
                os.path.dirname(model.trainer.checkpoint.dirpath), model.config.name
            )
...
def get_default_config(cfg_default):
    ...
    config = load_class(
        "get_cfg_defaults", paths=[cfg_default.replace("/", ".")], concat=False
    )()
...
def prepare_train_config(config):
    ...
    # Asserts
    assert (
        config.wandb.dry_run or config.wandb.entity is not ""
    ), "You need a wandb entity"
    assert (
        config.wandb.dry_run or config.wandb.project is not ""
...
def parse_test_file(ckpt_file, cfg_file=None):
    ...
    """
    assert ckpt_file.endswith(".ckpt") or ckpt_file.endswith(
        ".pth.tar"
    ), "You need to provide a .ckpt or .pth.tar file for checkpoint, not {}".format(
        ckpt_file
    )
    assert cfg_file is None or cfg_file.endswith(
        "yaml"
