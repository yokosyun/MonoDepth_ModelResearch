...
def load_depth(file):
    ...
    if file.endswith("npz"):
        ...
        return depth_png.astype(np.float) / 256.0
    else:
...
def calc_smoothness(inv_depths, images, num_scales):
    ...
    weights_x = [
        torch.exp(-torch.mean(torch.abs(g), 1, keepdim=True)) for g in image_gradients_x
    ]
    weights_y = [
        torch.exp(-torch.mean(torch.abs(g), 1, keepdim=True)) for g in image_gradients_y
    ]
...
def post_process_inv_depth(inv_depth, inv_depth_flipped, method="mean"):
    ...
    xs = torch.linspace(
        0.0, 1.0, W, device=inv_depth.device, dtype=inv_depth.dtype
    ).repeat(B, C, H, 1)
